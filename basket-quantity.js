(function(){
  ...
  function normalise(b){const out=[];const map=new Map();b.forEach(x=>{if(!x||!x.name)return;const k=key(x);let item=map.get(k);if(!item){item={name:x.name,price:x.price||'',category:x.category||'Nursery',quantity:0};map.set(k,item);out.push(item)}item.quantity+=Math.max(1,parseInt(x.quantity,10)||1)});return out}
  ...
  window.updateBasketQuantity=function(i,value){const b=normalise(read());const q=Math.max(0,parseInt(value,10)||0);if(!b[i])return;if(q===0)b.splice(i,1);else b[i].quantity=q;write(b);renderBasket()};
  window.removeItem=function(i){const b=normalise(read());b.splice(i,1);write(b);renderBasket()};
  window.sendOrder=function(){alert('MMS Nursery shop not currently open.\n\nWe are getting our nursery stock ready. Online ordering will be available soon.\n\nPlease check back shortly!')};
  function init(){ensureQtyFields();renderBasket();updateCounts()}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();window.addEventListener('storage',()=>{ensureQtyFields();renderBasket();updateCounts()});
})();
