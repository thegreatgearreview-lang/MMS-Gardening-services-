...
async function verifyAndConfirmOrder(orderId, env) {
  ...
  const internalHtml = `<!doctype html><html><body style="font-family:Arial,sans-serif;color:#183027;line-height:1.6"><div style="max-width:680px;margin:auto"><h2>New paid MMS Nursery order</h2><p><strong>Order:</strong> ${safe(order.orderId)}</p><p><strong>Customer:</strong> ${safe(order.firstName)} ${safe(order.lastName)}</p><p><strong>Email:</strong> ${safe(order.email)}</p><p><strong>Total paid:</strong> £${Number(order.amount).toFixed(2)}</p><p><strong>Items:</strong></p><ul>${order.items.map(x => `<li>${safe(x.name)} × ${x.quantity}</li>`).join('')}</ul></div></body></html>`;
  const [mail, internalMail] = await Promise.all([
    brevo('/smtp/email', env.BREVO_API_KEY, { sender: SENDER, to: [{ email: order.email, name: `${order.firstName} ${order.lastName}`.trim() || order.email }], subject: `MMS Nursery – Order ${order.orderId} confirmed`, htmlContent: customerHtml }),
    brevo('/smtp/email', env.BREVO_API_KEY, { sender: SENDER, to: [{ email: MMS_EMAIL, name: 'MMS Gardening Services' }], subject: `Paid MMS Nursery order ${order.orderId}`, htmlContent: internalHtml })
  ]);
  if (!mail.ok || !internalMail.ok) return false;
